...
Any node **MAY** define a local space bounding volume using a node's `boundingVolume` property. A bounding volume is a simple geometric shape, optionally transformed, that **SHOULD** completely enclose the node's content.
...
| `rotation`    | The bounding volume's unit quaternion rotation relative to the bounding volume's local coordinate system.
| `scale`       | The bounding volume's scale along the bounding volume's local X, Y, and Z axes.
| `translation` | The bounding volume's translation along the bounding volume's local X, Y, and Z axes.
|====
...
Boxes and spheres are the supported shapes for bounding volumes due to their simplicity and efficiency. Additional shapes **MAY** be allowed via extensions.

Additionally, changing the size of shapes **SHOULD** be preferred over changing the bounding volume's `scale`.

[NOTE]
.Implementation Note
====
If the bounding volume is transformed with a non-conformal transform (e.g. a transform with skew or non-uniform scale) an implementation may convert the bounding volume to an axis-aligned bounding box or other bounding volume that contains the transformed shape.
====

// TODO: only allow box and sphere but allow extensions to use other types? look at support across engines.
// TODO: add "exclusiveMinimum": 0.0 to scale or not?
// TODO: implementation behavior when computed node transform has skew / non-uniform scale / negative scale / zero scale

[NOTE]
.Implementation Note
====
Many engines use axis-aligned bounding boxes (AABBs) for mesh culling. When converting from bounding volumes to AABBs programmatically, the resulting AABB should include all extents of the original bounding volume, including any transformations.
====
...
// TODO: normal pointing outwards for other shape types

[options="header"]
|====
| `"box"`      | A box defined with a size vector, axis-aligned in local space with normals facing outwards.
| `"capsule"`  | A capsule defined by two "capping" spheres with potentially different radii, and height on the Y axis in local space with normals facing outwards.
| `"cylinder"` | A cylinder with potentially different radii at each end, and height on the Y axis in local space with normals facing outwards.
| `"plane"`    | A plane centered at the origin in local space, optionally with finite extents, with normal along the +Y axis in local space.
| `"sphere"`   | A sphere shape defined with a radius equal in all directions with normals facing outwards.
|====


For the `"box"`, `"capsule"`, `"cylinder"`, `"plane"`, and `"sphere"` shape types, a sub-object with a key of matching the shape type **MAY** be used to specify the size parameters of the shape. When the `type` of a shape refers to a built-in shape type, the shape must not supply a sub-object for a different shape type: for example, a shape whose `type` is "box" must not have a `sphere` property defined. For each of those five, the shape is centered at the origin in local space.
