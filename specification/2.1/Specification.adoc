...
Any node **MAY** define a local space bounding volume using a node's `boundingVolume` property. A bounding volume is a simple geometric shape, optionally transformed, that **SHOULD** completely enclose the node's geometry.
...
.Implementation Note
====
* TODO: Switch this to be the inverse - define what transforms are allowed.
* TODO: be careful about ISO wording with *may*
If the bounding volume is transformed with a non-conformal transform (e.g. a transform with skew or non-uniform scale) an implementation may convert the bounding volume to an axis-aligned bounding volume that contains the transformed shape.
====
...
[options="header"]
|====
| `"box"`      | A box defined with a size vector, axis-aligned in local space with hard normals facing outwards.
| `"capsule"`  | A capsule defined by two "capping" spheres with potentially different radii, and height on the Y axis in local space with smooth normals facing outwards.
| `"cylinder"` | A cylinder with potentially different radii at each end, and height on the Y axis in local space with hard normals facing outwards on the caps and smooth normals facing outwards on the sides.
...
| `"sphere"`   | A sphere shape defined with a radius equal in all directions with smooth normals facing outwards.
|====
