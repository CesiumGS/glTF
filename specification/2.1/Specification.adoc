...
Boxes and spheres are the recommended shapes for bounding volumes due to their simplicity and efficiency. Additionally, changing the size of shapes **SHOULD** be preferred over changing the bounding volume's `scale`.
...
[options="header"]
|====
| `"box"`      | A box defined with a size vector, axis-aligned in local space.
| `"capsule"`  | A capsule defined by two "capping" spheres with potentially different radii, and height on the Y axis in local space.
| `"cylinder"` | A cylinder with potentially different radii at each end, and height on the Y axis in local space.
| `"plane"`    | A plane centered at the origin in local space, with normal along the +Y axis in local space, optionally with finite extents. 
| `"sphere"`   | A sphere shape defined with a radius equal in all directions.
|====
