...
This extension declares the Coordinate Reference System (CRS) in which a glTF 2.1 asset was authored. The CRS is provided as a Well-Known ID (WKID), often an [EPSG](https://epsg.org/home.html) code.
...
- `wkid` is a unique integer that identifies the CRS.
- `vcsWkid` is an optional unique integer that identifies the vertical CRS when `wkit` refers to a horizontal CRS.
- `epoch` is an optional epoch for coordinates that are referenced to a dynamic CRS such as WGS 84. Expressed as a decimal year (e.g. `"2019.81"`). See [WKT representation of coordinate epoch and coordinate metadata](http://docs.opengeospatial.org/is/18-010r7/18-010r7.html#128) for more details.

## Examples

An asset defined in [WGS 84](https://epsg.org/ellipsoid_7030/WGS-84.html) geocentric coordinates with defined epoch:
